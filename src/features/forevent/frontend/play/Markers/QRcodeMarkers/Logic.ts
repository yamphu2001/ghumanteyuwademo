...
// --- Update QRcodeMarkerData Interface ---
export interface QRcodeMarkerData {
  id: string;
  name: string;
  lat: number;
...
// --- Update fetchMuseumMarkers Function ---
export async function fetchMuseumMarkers(eventId: string): Promise<QRcodeMarkerData[]> {
  try {
    const snap = await getDocs(collection(db, "events", eventId, "qrcodemarkers"));
    ...
    return snap.docs.map((d) => {
      ...
      return {
        ...
        eventId: eventId,
        points: Number(raw.points ?? 0), // ADD THIS LINE
      };
    });
  } catch (e: any) {
...
export function projectMarkers(
  ...
  return markers.map((m) => {
    ...
    return {
      ...
      popupImage: m.popupImage, popupText: m.popupText,
