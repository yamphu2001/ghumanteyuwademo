...
function SingleBoundary({ map, config }: { map: maplibregl.Map; config: ServiceConfig }) {
  ...
  useEffect(() => {
    ...
    if (hasImage) {
      // FIX: Check if height exists to decide whether to mount 3D roof marker OR ground 2D marker
      if (config.height && config.height > 0) {
        const container = map.getContainer();
        topMarkerEl = document.createElement('div');
        topMarkerEl.style.position = 'absolute';
        topMarkerEl.style.zIndex = '10';
        topMarkerEl.style.cursor = 'pointer';
        topMarkerEl.style.backfaceVisibility = 'hidden';
        topMarkerEl.style.transformStyle = 'preserve-3d';
        ...
        // Flat 2D ground surface representation fallback
