...
import { Settings, Gift, Home, Scan, Plus, X } from 'lucide-react';
import { useRouter } from "next/navigation";
import dynamic from 'next/dynamic'; // 1. Added for the overlay
...
// 2. Import the scanner as a component, NOT a page.
// This ensures it only loads when the button is clicked.
const QRScannerOverlay = dynamic(() => import('@/features/frontend/play/qrscanner/qrscanner'), { 
  ssr: false 
});
...
export default function RedesignedOneHandedMenu() {
  ...
  const [activeScanner, setActiveScanner] = useState(false); // 3. State to control the overlay
  const router = useRouter();
  const { handPreference } = useUIPreference();
  
  const layout = getLayout(handPreference || "center");

  return (
    <>
      {/* 4. THE SCANNER COMPONENT OVERLAY */}
      <AnimatePresence>
        {activeScanner && (
          <motion.div 
            key="scanner-overlay" // Adding a key helps Framer Motion track the unmount
          ...
          >
            ...
            <div className="absolute bottom-[8%] left-1/2 -translate-x-1/2 z-[10000]">
              <button 
                onClick={() => setActiveScanner(false)} // This trigger unmounts QRScanner
          ...
          <AnimatePresence mode="wait">
            {!isOpen ? (
              ...
              <motion.div key="menu-content">
                ...
                {/* 5. THE SCAN BUTTON (Removed router.push) */}
                <motion.button
                  className={`${styles.scanButton} ${styles[layout.alignment]}`}
                  onClick={() => { 
                    setIsOpen(false); 
                    setActiveScanner(true); // Triggers the overlay instead of redirecting
                  }}
                  initial={{ x: 0, y: 0, scale: 0 }}
