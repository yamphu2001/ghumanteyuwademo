    ...
    <div className="max-w-5xl mx-auto p-6 bg-slate-50 min-h-screen rounded-2xl">
      {isMapFullScreen && (
        <div className="fixed inset-0 bg-slate-900 z-[9999] flex flex-col">
          <div className="p-4 bg-slate-800 flex justify-between items-center text-white">
            ...
            <button onClick={() => setIsMapFullScreen(false)} className="px-6 py-2 bg-blue-600 rounded-lg font-bold">
              Confirm Shape
            </button>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden grid grid-cols-1 md:grid-cols-12 border border-slate-200">
        <div className="md:col-span-5 p-8 border-r border-slate-100 space-y-6">
          <div className="flex justify-between items-center">
            ...
            {editingId && (
              <button onClick={resetForm} className="text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded">
                Cancel Edit
              </button>
            )}
          </div>
          ...
          <div className="space-y-3 pt-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-[2] py-4 bg-blue-600 disabled:bg-slate-200 text-white rounded-xl font-bold"
              >
                {isSaving ? "Saving..." : editingId ? "Update Area" : "Save New Area"}
              </button>
              <button onClick={resetForm} className="flex-1 bg-emerald-50 text-emerald-600 rounded-xl font-bold hover:bg-emerald-100 transition">
                + New
              </button>
            </div>
          </div>
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-lg border border-slate-200 p-6">
        <h3 className="text-lg font-bold mb-4 text-slate-700">
          Existing Service Areas ({services.length})
        </h3>
