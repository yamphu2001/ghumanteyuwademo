...
export default function QRCodeMarkers({ map, eventId }: { map: any, eventId: string }) {
  const qrMarkersRef = useRef<maplibregl.Marker[]>([]);

  useEffect(() => {
    if (!map || !eventId) return;

    const loadQRMarkers = async () => {
      qrMarkersRef.current.forEach(m => m.remove());
      qrMarkersRef.current = [];

      const colRef = collection(db, 'events', eventId, 'qrcodemarkers');
      ...
      snapshot.forEach((doc) => {
        ...
        // --- STYLING TO MATCH image_47f85d.png ---
        const el = document.createElement('div');
        el.style.width = '50px';
        el.style.height = '50px';
        el.style.cursor = 'pointer';
        
        // 1. Create the Teardrop Shape
        el.style.backgroundColor = 'white'; // This acts as the white border
        el.style.borderRadius = '50% 50% 50% 0';
        el.style.transform = 'rotate(-45deg)'; // Rotate to point the sharp corner down
        ...
        imgInner.style.width = '42px'; // Slightly smaller than parent to show "border"
        ...
        // 3. Counter-rotate the image so it is upright
        ...
        const marker = new maplibregl.Marker({ 
          element: el,
          anchor: 'bottom' // Align the point to the coordinate
        })
        ...
        qrMarkersRef.current.push(marker);
      });
    };
