...
//     "loading" | "no_prize" | "ready" | "result" | "already_spin" | "no_stock"
//   >("loading");
  ...
  return (
    <motion.div animate={controls} style={{ width: size, height: size }} className="relative">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="drop-shadow-xl">
        ...
        <circle cx={cx} cy={cy} r={6} fill="#dc2626" /> {/* Red center dot */}
      </svg>
    </motion.div>
  );
}
...
export default function PlayerRoulettePage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  if (stage === "no_prize" || stage === "no_stock" || stage === "already_spin" || (stage === "result" && winner)) {
    const config = {
      no_prize: { icon: <Gift size={40} />, title: "No Access", body: "Claim your map prize first.", btnLabel: "Back to Map", color: "black" },
      no_stock: { icon: <Gift size={40} className="text-red-600" />, title: "Empty Pool", body: "All prizes have been claimed.", btnLabel: "Back to Map", color: "black" },
      already_spin: { icon: <Lock size={40} />, title: "Completed", body: "You have already used your spin.", btnLabel: "Back to Map", color: "black" },
      result: { icon: <Trophy size={40} className="text-red-600" />, title: "You Won!", body: winner?.name || "", btnLabel: "Done", color: "red-600" }
    };
    ...
    return (
      <FullScreen bg="white">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white border-4 border-black p-8 text-center max-w-sm w-full shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <button onClick={() => window.location.href = `/eventsmaker/${eventId}`} className="w-full py-4 bg-black text-white font-black uppercase tracking-tighter hover:bg-red-600 transition-colors">
            {current.btnLabel}
          </button>
        </motion.div>
      </FullScreen>
  ...
  return (
    <main className="fixed inset-0 z-[9999] bg-white flex flex-col items-center justify-between p-6">
      ...
      <div className="w-full flex items-center justify-between">
        <button onClick={() => window.location.href = `/eventsmaker/${eventId}`} className="p-2 border-2 border-black hover:bg-black hover:text-white transition-all">
          <ChevronLeft size={24} />
        </button>
