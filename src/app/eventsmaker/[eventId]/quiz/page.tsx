...
export default function QuizPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        // 2. Fetch Progress check (80% rule) - UPDATED DIRECT PATH (No userInfo layer)
        const progressRef = ref(rtdb, `eventsProgress/${eventId}/${user.uid}/progress`);
        const snapshot = await get(progressRef);
        const progressVal = snapshot.val() || "0%";

        // Bulletproof parsing for both string "80%" and number 80 formats
        const currentPct = typeof progressVal === "string"
          ? parseInt(progressVal.replace("%", ""), 10)
          : Number(progressVal);
  ...
  const saveToFirebase = async (finalScore: number) => {
    ...
    try {
      ...
      // 2. Firestore update (Appends quizPoints field to player_log subcollection)
      const playerLogRef = doc(db, "events", eventId, "player_log", user.uid);
      await updateDoc(playerLogRef, {
        quizPoints: finalScore
      });
      console.log("🔥 Quiz points updated in Firestore!");
