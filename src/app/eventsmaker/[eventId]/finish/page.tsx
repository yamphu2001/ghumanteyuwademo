      ...
      if (user && eventId) {
        try {
          ...
          if (playerLogSnapshot.exists()) {
            ...
            // --- UPDATED TIME LOGIC START ---
            if (logData.totalTime) {
              // Directly read and use totalTime if it's already generated and stored in Firestore
              calculatedTimeTaken = logData.totalTime;
            } else if (logData.startat && logData.finishat) {
              // Fallback calculation if totalTime field isn't present yet
              const startTime = new Date(logData.startat).getTime();
              const finishTime = new Date(logData.finishat).getTime();
              ...
              if (!isNaN(startTime) && !isNaN(finishTime)) {
                ...
                if (diffMs >= 0) {
                  ...
                  // Write the calculated time back to Firestore for faster next-time loads
                  await updateDoc(playerLogRef, {
                    totalTime: calculatedTimeTaken
                  });
                }
