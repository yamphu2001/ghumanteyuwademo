...
//               // Read completion flags from player_log
...
//   const showRoulette = endStats.quizCompleted; // only show after quiz done
...
//                   QR: <span className="text-black">{endStats.qrPoints}</span>
//                 </p>
//                 <p className="text-[9px] font-bold uppercase tracking-wider text-gray-500">
//                   Quiz: <span className="text-black">{endStats.quizPoints}</span>
//                 </p>
//               </div>
//             </div>
//           </div>

//           {/* DYNAMIC PRIZE REVEAL AREA */}
//           {endStats.wonPrize && (
//             <div className="pt-2 border-t border-dashed border-gray-200 animate-fadeIn">
//               <p className="text-[9px] md:text-[15px] font-bold uppercase tracking-[0.3em] text-gray-500 mb-1">Your Reward</p>
//               <p className="text-3xl md:text-2xl font-black uppercase text-black tracking-tight drop-shadow-sm">
//                 🎁 {endStats.wonPrize}
//               </p>
//             </div>
//           )}
...
//           {/* Quiz Button */}
//           <button
//             onClick={() => { if (quizClickable) window.location.href = `/eventsmaker/${eventId}/quiz`; }}
...
//           {/* Roulette Button — only shown after quiz is done */}
//           {showRoulette && (
//             <button
//               onClick={() => { if (rouletteClickable) window.location.href = `/eventsmaker/${eventId}/roulette`; }}
...
const FinishPage = () => {
  const { eventId } = useParams();
  const hasMarkedFinish = useRef(false); // ← prevent double-write
  const [endStats, setEndStats] = useState<any>({
    uid: null,
    totalTime: "N/A",
    completionRate: 0,
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      if (user && eventId) {
        // ✅ Mark player as finished — once only
        if (!hasMarkedFinish.current) {
          hasMarkedFinish.current = true;
          const { markPlayerFinished } = await import("../play/_checkFinished");
          await markPlayerFinished(eventId as string, user.uid);
        }
