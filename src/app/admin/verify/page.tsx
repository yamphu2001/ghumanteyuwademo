...
interface PrizeClaim {
  ...
  correctanswers: number;
  visitedCount:   number;
  durationDisplay: string;  // "1m 38s" — exact, no rounding
  finished:       boolean;
  claimedAt:      any;
}

// ── Parse the plain-text QR format ──────────────────────────────────────────
// "User: TEST123\nPhone: 9812300000\nCorrect Answers: 2\n..."
function parseQRText(text: string): Partial<PrizeClaim> | null {
  try {
    ...
    // Must have at least a username to be valid
    if (!lines["user"]) return null;

    return {
      username:        lines["user"]              ?? "Unknown",
      phone:           lines["phone"]             ?? "N/A",
      correctanswers:  parseInt(lines["correct answers"] ?? "0", 10),
      visitedCount:    parseInt(lines["locations visited"] ?? "0", 10),
      durationDisplay: lines["time taken"]        ?? "0s",
      finished:        (lines["finished"] ?? "").toLowerCase() === "yes",
    };
  } catch {
    return null;
  }
...
export default function AdminPrizeScanner() {
  ...
  // Live history from Firestore
  useEffect(() => {
    const q = query(collection(db, "prizeClaims"), orderBy("claimedAt", "desc"));
    const unsub = onSnapshot(q, (snap) => {
      setHistory(snap.docs.map((d) => d.data() as PrizeClaim));
    });
    return () => unsub();
    ...
    scanner.render(
      async (decodedText) => {
        ...
        if (!parsed || !parsed.username) {
          setStatus("❌ Invalid QR — no player data found");
          return;
        }

        // Use username as the Firestore doc key (unique per player)
        const claimKey = parsed.username.trim().toUpperCase();
        const prizeRef = doc(db, "prizeClaims", claimKey);
        const existing = await getDoc(prizeRef);

        if (existing.exists()) {
          setStatus(`⚠️ ${parsed.username} already claimed their prize`);
          setLastResult(existing.data() as PrizeClaim);
          return;
        }
        ...
        await setDoc(prizeRef, prizeData);
        setLastResult(prizeData);
        setStatus(`✅ Prize recorded for ${parsed.username}!`);
      },
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-2 mb-6">
          {(["scanner", "history"] as const).map((tab) => (
            ...
            >
              {tab === "scanner" ? "📷 Scanner" : `📋 History (${history.length})`}
            </button>
          ))}
        </div>
            ...
            {lastResult && (
              <div className="mt-6 bg-white p-5 rounded-2xl shadow w-full max-w-md">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <p><span className="font-semibold">Score:</span>    {lastResult.correctanswers}</p>
                  <p><span className="font-semibold">Visited:</span>  {lastResult.visitedCount} markers</p>
                  <p><span className="font-semibold">Duration:</span> {lastResult.durationDisplay}</p>
                  <p><span className="font-semibold">Finished:</span> {lastResult.finished ? "✅ Yes" : "❌ No"}</p>
                </div>
              </div>
            )}
            ...
            {history.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      {["Name","Phone","Score","Visited","Duration","Finished","Claimed At"].map((h) => (
                  ...
                  <tbody>
                    {history.map((claim, i) => (
                      <tr key={claim.username} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                        <td className="px-4 py-3 font-medium">{claim.username}</td>
                        <td className="px-4 py-3 text-gray-600">{claim.phone}</td>
                        <td className="px-4 py-3">{claim.correctanswers}</td>
                        <td className="px-4 py-3">{claim.visitedCount}</td>
                        <td className="px-4 py-3">{claim.durationDisplay}</td>
                        <td className="px-4 py-3">{claim.finished ? "✅" : "❌"}</td>
                        <td className="px-4 py-3 text-gray-500 text-xs">{formatDate(claim.claimedAt)}</td>
                      </tr>
                    ))}
